...
describe('Card Class', () => {
  ...
  test('Card Class Should have A method, cardInfo', () => {
    ...
    expect(cardInfo).toEqual('King of Diamonds')
  })
  test('Card Class Should have a getter for cardInfo', () =>{
    const newCard = new Card('Diamonds', 'King');
    const cardInfo = newCard.getCardInfo()
    expect(newCard.cardInfo).toBe(`${newCard.type} of ${newCard.suit}`)
    expect(cardInfo).toEqual(newCard.cardInfo)
